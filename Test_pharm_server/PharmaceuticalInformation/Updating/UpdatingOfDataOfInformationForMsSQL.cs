...
namespace PharmaceuticalInformation.Updating
{
    public class UpdatingOfDataOfInformationForMsSQL : UpdatingOfDataOfInformation
    {
        ...
        //    PhrmInf.price_list.Join(PhrmInf.Pharmacies.Where(ph => ph.Is_deleted), pl => pl.Id_Pharmacy, ph => ph.Id_Pharmacy, (pl, ph) => pl).Delete();
        //    PhrmInf.price_list.Where(pl => !PhrmInf.Products.Where(pr => pr.Id_Product == pl.Id_Product).Any()).Delete();
